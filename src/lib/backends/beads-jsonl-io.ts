...
export function resolveDepsPath(repoPath: string): string {
  return join(repoPath, ".beads", "deps.jsonl");
}
...
export async function writeJsonlFile(
  ...
  await writeJsonlRecords(filePath, records);
}

// ── Generic JSONL I/O ─────────────────────────────────────────

/**
 * Read and parse all records from a JSONL file (generic version).
 * Returns an empty array if the file doesn't exist or is empty.
 */
...
/**
 * Write all records to a JSONL file (generic version).
 * Creates the parent directory if it does not exist.
 */
export async function writeJsonlRecords<T>(
  filePath: string,
  records: T[],
): Promise<void> {
