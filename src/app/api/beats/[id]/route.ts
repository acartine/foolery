...
export async function GET(
  ...
  if (isSuppressibleError(error)) {
    ...
    logApiError({ method: "GET", path: `/api/beats/${id}`, status: 503, error: DEGRADED_ERROR_MESSAGE });
    return NextResponse.json({ error: DEGRADED_ERROR_MESSAGE }, { status: 503 });
  }

  if (result.error?.code === "NOT_FOUND" || isNotFoundError(error)) {
    logApiError({ method: "GET", path: `/api/beats/${id}`, status: 404, error });
    return NextResponse.json({ error }, { status: 404 });
  }
  const getStatus = backendErrorStatus(result.error);
  logApiError({ method: "GET", path: `/api/beats/${id}`, status: getStatus, error });
...
export async function PATCH(
  ...
  if (!parsed.success) {
    logApiError({ method: "PATCH", path: `/api/beats/${id}`, status: 400, error: "Validation failed" });
    return NextResponse.json(
  ...
  if (!result.ok) {
    ...
    logApiError({ method: "PATCH", path: `/api/beats/${id}`, status: updateStatus, error: result.error?.message });
    return NextResponse.json(
...
export async function DELETE(
  ...
  if (!result.ok) {
    ...
    logApiError({ method: "DELETE", path: `/api/beats/${id}`, status: deleteStatus, error: result.error?.message });
    return NextResponse.json(
      { error: result.error?.message },
      { status: deleteStatus },
    );
