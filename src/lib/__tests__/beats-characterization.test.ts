/**
 * Characterization tests for the beats subsystem.
 *
...
describe("beat-sort: naturalCompare", () => {
  it("SRT-001: sorts numeric segments numerically", () => {
    expect(naturalCompare("beat-2", "beat-10")).toBeLessThan(0);
    expect(naturalCompare("beat-10", "beat-2")).toBeGreaterThan(0);
  });
  ...
  it("SRT-003: returns 0 for equal strings", () => {
    expect(naturalCompare("beat-5", "beat-5")).toBe(0);
...
describe("beat-sort: compareBeatsByPriorityThenState", () => {
  ...
  it.todo("SRT-008: compareBeatsByHierarchicalOrder delegates to naturalCompare");
});

// ===========================================================================
// 4. Beat Hierarchy (HIR-*)
// ===========================================================================

describe("beat-hierarchy: buildHierarchy", () => {
  it("HIR-001: flat list of root beats has depth 0", () => {
    const beats = [
      makeBeat({ id: "a" }),
      makeBeat({ id: "b" }),
    ];
...
describe("ready-ancestor-filter: filterByVisibleAncestorChain", () => {
  it.todo("RAF-001: top-level beats always visible");
...
describe("schemas: createBeatSchema", () => {
  it.todo("SCH-001: requires title");
  it.todo("SCH-002: defaults type to task");
  it.todo("SCH-003: defaults priority to 2");
});

describe("schemas: updateBeatSchema", () => {
  it.todo("SCH-004: all fields optional");
});

describe("schemas: enum validation", () => {
  it.todo("SCH-005: beatTypeSchema accepts all valid types");
  it.todo("SCH-006: beatStatusSchema accepts all valid statuses");
  it.todo("SCH-007: beatPrioritySchema accepts 0-4");
  it.todo("SCH-008: queryBeatSchema requires expression");
});
...
describe("api: client wrappers", () => {
  it.todo("API-001: fetchBeats builds correct query string");
  it.todo("API-002: fetchReadyBeats calls /api/beats/ready");
  it.todo("API-003: createBeat sends POST with body");
  it.todo("API-004: mergeBeats sends POST with survivorId/consumedId");
  it.todo("API-005: fetchBeatsFromAllRepos fans out and merges");
...
describe("app-store: beat filter state", () => {
...
describe("bd.ts: updateBeat label operations", () => {
