/**
 * Agent history tests: session detail retrieval, workflow states, worktrees.
 */
...
describe("readAgentHistory sessions", () => {
  beforeEach(async () => {
  ...
  it("returns selected beat sessions with prompt source metadata", async () => {
    await writeLog(tempDir, "repo-a/2026-02-20/term-c.jsonl", [
      ...
      },
      {
        ...
        sessionId: "term-c", prompt: "Initial prompt", source: "initial",
      },
      {
    ...
    const history = await readAgentHistory({
      logRoot: tempDir, beatId: "foo-1", beatRepoPath: "/tmp/repo-a",
    });
    ...
    expect(session?.entries.map((entry) => entry.kind)).toEqual([
      "session_start", "prompt", "response", "prompt", "session_end",
    ]);
  ...
  it("captures workflow states from beat_state entries", async () => {
    await writeLog(tempDir, "repo-a/2026-02-20/term-workflow.jsonl", [
      ...
      },
      {
        ...
        sessionId: "term-workflow", prompt: "Prompt", source: "initial",
      },
      {
        kind: "beat_state", ts: "2026-02-20T13:20:02.000Z",
        sessionId: "term-workflow", beatId: "foo-1",
        state: "ready_for_plan_review", phase: "after_prompt", iteration: 1,
      },
      {
        kind: "session_end", ts: "2026-02-20T13:20:03.000Z",
        sessionId: "term-workflow", status: "completed", exitCode: 0,
      },
    ]);
  ...
  it("annotates each prompt with prompt number and workflow state", async () => {
    await writeLog(tempDir, "repo-a/2026-02-20/term-workflow-multi.jsonl", [
      {
        kind: "session_start", ts: "2026-02-20T13:25:00.000Z",
        sessionId: "term-workflow-multi", interactionType: "take",
        repoPath: "/tmp/repo-a", beatIds: ["foo-1"],
      },
      {
      ...
      },
      {
        ...
        sessionId: "term-workflow-multi", beatId: "foo-1",
        state: "plan_review", phase: "before_prompt", iteration: 2,
      },
      {
      ...
      },
      {
        ...
        sessionId: "term-workflow-multi", status: "completed", exitCode: 0,
      },
    ]);
    ...
    expect(prompts?.map((entry) => entry.promptNumber)).toEqual([1, 2]);
    expect(prompts?.map((entry) => entry.workflowState)).toEqual([
  ...
  it("includes direct sessions and prompt metadata for selected beats", async () => {
    await writeLog(tempDir, "repo-a/2026-02-20/direct-a.jsonl", [
      ...
      },
      {
        ...
        sessionId: "direct-a", prompt: "Review prompt", source: "initial",
      },
      {
  ...
  it("treats worktree paths as the same repo when filtering", async () => {
    ...
    await writeLog(tempDir, "repo-a/2026-02-20/worktree.jsonl", [
      ...
      },
      {
        ...
        prompt: "ID: worktree-beat\nTitle: Worktree beat", source: "initial",
      },
      {
        kind: "session_end", ts: "2026-02-20T14:02:02.000Z",
        sessionId: "worktree-session", status: "completed", exitCode: 0,
      },
    ]);
    ...
    await writeLog(tempDir, "repo-a/2026-02-20/sibling-worktree.jsonl", [
      {
        ...
        repoPath: "/tmp/foolery-wt-feature-1", beatIds: ["sibling-beat"],
      },
    ]);

    const history = await readAgentHistory({ logRoot: tempDir, repoPath });
    ...
    expect(sessionHistory.sessions[0]?.entries.map((entry) => entry.kind)).toEqual([
      "session_start", "prompt", "session_end",
    ]);
  });
  ...
  it("returns sessions when queried with beatId matching beatIds in logs", async () => {
    await writeLog(tempDir, "repo-a/2026-02-20/session-query.jsonl", [
      ...
      },
      {
        ...
        sessionId: "session-query", prompt: "Do the work", source: "initial",
      },
      {
        kind: "session_end", ts: "2026-02-20T17:01:00.000Z",
        sessionId: "session-query", status: "completed", exitCode: 0,
      },
    ]);

    const history = await readAgentHistory({
      logRoot: tempDir, beatId: "foo-query", beatRepoPath: "/tmp/repo-a",
    });
